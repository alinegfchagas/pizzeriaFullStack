    ...
    constructor(orderDatabase, idGenerator) {
        ...
        this.createOrder = async (input) => {
            ...
            if (pizzasInput.length === 0) {
                throw new ParamsError_1.ParamsError("Pedido vazio! Informe pelo menos uma pizza");
            }
            const pizzas = pizzasInput.map((pizza) => {
                if (pizza.quantity <= 0) {
                    throw new ParamsError_1.ParamsError("Quantidade de pizza inválida! A quantidade mínima é 1");
                }
                return {
                    ...pizza,
                    price: 0
            ...
            for (let pizza of pizzas) {
                ...
                if (!price) {
                    throw new NotFoundError_1.NotFoundError("Pizza não existe");
                }
                pizza.price = price;
            }
        ...
        this.getOrders = async () => {
            ...
            for (let orderDB of ordersDB) {
                const order = new Orders_1.Order(orderDB.id, []);
